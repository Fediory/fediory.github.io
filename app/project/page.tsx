  ...
  return (
    <main>
      ...
      <div className="site-wrap publication-page">
        ...
        <section className="section publication-section"><div className="project-grid project-page-grid">{projects.map((project) => <article className="project-card" key={project.title}><a className="project-visual" href={project.links[0][1]} target={project.links[0][1].startsWith("http") ? "_blank" : undefined} rel={project.links[0][1].startsWith("http") ? "noreferrer" : undefined} aria-label={`${project.title} project`}><img src={project.image} alt={zh ? project.imageAltZh : project.imageAlt} loading="lazy" decoding="async" /><span className="project-image-index">{project.index}</span></a><div className="project-card-body"><span className="project-index">{zh ? `项目 ${project.index}` : `PROJECT ${project.index}`}</span><strong>{zh ? project.titleZh : project.title}</strong><p>{zh ? project.descriptionZh : project.description}</p><div className="project-actions">{project.links.map(([label, href]) => <a key={href} href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined}>{zh && label === "Paper Page ↗" ? "论文主页 ↗" : zh && label === "Project ↗" ? "项目 ↗" : zh && label === "Demo ↗" ? "演示 ↗" : label}</a>)}{project.tag && <span>{zh ? project.tagZh : project.tag}</span>}</div>{project.sourceHref && <a className="project-image-credit" href={project.sourceHref} target="_blank" rel="noreferrer">{zh ? project.sourceLabelZh : project.sourceLabel}</a>}</div></article>)}</div></section>
      </div>
